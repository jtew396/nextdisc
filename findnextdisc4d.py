...
def main():
    ...
    for disc in disc_data:
        attribute_diffs = [15.0, 15.0, 15.0, 15.0]
        for i in range(len(disc_data[disc])):
            min_diff = attribute_diffs[i]
            for owned_disc in discs_owned:
                diff = abs(float(disc_data[owned_disc][i]) - float(disc_data[disc][i]))
                if diff < min_diff:
                    min_diff = diff
            attribute_diffs[i] = min_diff
